...
(function () {
  ...
  class V8U {
    ...
    loop(fn) { let raf, alive = true; const tick = t => { if (!alive) return; if (this._fc) { for (let i = 0; i < this._fc.length; i++) this._fc[i](); } fn(t); raf = requestAnimationFrame(tick); }; raf = requestAnimationFrame(tick); this.cleanup.push(() => { alive = false; cancelAnimationFrame(raf); }); return () => { alive = false; cancelAnimationFrame(raf); }; }
...
@fragment fn fs(@builtin(position) frag:vec4<f32>)->@location(0) vec4<f32>{
  ...
  return vec4<f32>(col*u.fade, u.fade);
    ...
    mountCarousel(stage, a) {
      ...
      this.particleField(wrap, a, { count: 30, isView: () => inView });
      ...
      this.loop(() => { cosmosRot += (cosmosTarget - cosmosRot) * 0.07 + 0.0016; auroraPh += 0.012; if (dominoing) cosmosFade = Math.max(0, cosmosFade - 0.02); const w = cosmos.clientWidth, h = cosmos.clientHeight, cx = w / 2, cy = h / 2; if (!w || !h || !inView) return; xctx.clearRect(0, 0, w, h); const gR = Math.min(w, h) * 0.95; xctx.globalAlpha = 1;
