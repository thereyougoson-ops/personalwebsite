...
@fragment fn fs(@builtin(position) frag:vec4<f32>)->@location(0) vec4<f32>{
  ...
  return vec4<f32>(col*u.fade, u.fade);
    ...
    mountCarousel(stage, a) {
      ...
      const wrap = this.h('div', 'position:absolute;inset:0;overflow:hidden;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:clamp(16px,3vw,40px);perspective:1400px;background:radial-gradient(circle at 50% 40%,#0d0c18,#06060d 78%);');
      this.particleField(wrap, a, { count: mob ? 14 : 30, isView: () => inView });
      ...
      // cosmic backdrop — spinning galaxy + Earth at the ring's center (Canvas-2D fallback path)
      const cosmos = this.h('canvas', 'position:absolute;inset:0;width:100%;height:100%;z-index:1;pointer-events:none;'); wrap.appendChild(cosmos); const xctx = this.fitCanvas(cosmos);
      const STAR_N = (window.innerWidth < 700 ? 360 : 720); // perf: lighter galaxy on phones (invisible quality change)
      const gstars = []; for (let gi = 0; gi < STAR_N; gi++) { const r = Math.pow(Math.random(), 0.5); const arm = Math.floor(Math.random() * 3); gstars.push({ r, a: r * 7 + arm * 2.094 + (Math.random() - 0.5) * 0.6, sz: Math.random() * 1.5 + 0.2, b: Math.random() * 0.6 + 0.4, c: Math.random() < 0.18 ? '#ffd9a0' : (Math.random() < 0.5 ? '#bcd0ff' : '#ffffff') }); }
      const nebula = []; for (let ni = 0; ni < 7; ni++) { nebula.push({ r: 0.25 + Math.random() * 0.55, a: Math.random() * 6.28, sz: 0.2 + Math.random() * 0.25, c: ['120,90,220', '60,120,220', '200,80,160', '80,160,140'][ni % 4] }); }
      const planets = [{ dist: 0.30, ang: 0.6, sp: 0.6, r: 0.022, col: '#c1664a', ring: 0 }, { dist: 0.42, ang: 3.4, sp: 0.4, r: 0.042, col: '#d9b87a', ring: 1 }, { dist: 0.20, ang: 1.8, sp: 1.1, r: 0.013, col: '#b9c0cc', ring: 0 }];
      let cosmosRot = 0, cosmosTarget = 0, cosmosFade = 1, auroraPh = 0; const dStreaks = [];
      // PHONES: skip the per-frame galaxy/Earth raymarch (it ran the section at single-digit fps) — paint
      // a cheap static nebula gradient + a one-time sparse starfield instead. Ring/cards/domino unchanged.
      if (mob) {
        wrap.style.background = 'radial-gradient(120% 90% at 50% 42%,rgba(120,90,220,.20),rgba(13,12,24,0) 55%),radial-gradient(80% 70% at 50% 66%,rgba(60,120,200,.14),rgba(13,12,24,0) 60%),radial-gradient(circle at 50% 40%,#0d0c18,#06060d 80%)';
        const drawStars = () => { const w = cosmos.clientWidth, h = cosmos.clientHeight; if (!w || !h) return; xctx.clearRect(0, 0, w, h); for (let i = 0; i < 80; i++) { const x = Math.random() * w, y = Math.random() * h, rr = Math.random() * 1.2 + 0.3; xctx.globalAlpha = Math.random() * 0.55 + 0.35; xctx.fillStyle = Math.random() < 0.18 ? '#ffd9a0' : (Math.random() < 0.5 ? '#bcd0ff' : '#ffffff'); xctx.beginPath(); xctx.arc(x, y, rr, 0, 6.28); xctx.fill(); } xctx.globalAlpha = 1; };
        setTimeout(drawStars, 90); setTimeout(drawStars, 600); const onR = () => setTimeout(drawStars, 60); window.addEventListener('resize', onR); this.cleanup.push(() => window.removeEventListener('resize', onR));
      }
      this.loop(() => { if (mob) return; cosmosRot += (cosmosTarget - cosmosRot) * 0.07 + 0.0016; auroraPh += 0.012; if (dominoing) cosmosFade = Math.max(0, cosmosFade - 0.02); const w = cosmos.clientWidth, h = cosmos.clientHeight, cx = w / 2, cy = h / 2; if (!w || !h || !inView) return; xctx.clearRect(0, 0, w, h); const gR = Math.min(w, h) * 0.95; xctx.globalAlpha = 1;
        const cg = xctx.createRadialGradient(cx, cy, 0, cx, cy, gR * 0.75); cg.addColorStop(0, 'rgba(180,150,255,' + (0.4 * cosmosFade) + ')'); cg.addColorStop(0.4, 'rgba(110,90,200,' + (0.16 * cosmosFade) + ')'); cg.addColorStop(1, 'transparent'); xctx.fillStyle = cg; xctx.fillRect(0, 0, w, h);
        xctx.globalCompositeOperation = 'lighter'; nebula.forEach(nb => { const a2 = nb.a + cosmosRot * 0.3; const x = cx + Math.cos(a2) * nb.r * gR, y = cy + Math.sin(a2) * nb.r * gR * 0.45; const ng = xctx.createRadialGradient(x, y, 0, x, y, nb.sz * gR); ng.addColorStop(0, 'rgba(' + nb.c + ',' + (0.16 * cosmosFade) + ')'); ng.addColorStop(1, 'transparent'); xctx.fillStyle = ng; xctx.fillRect(0, 0, w, h); }); xctx.globalCompositeOperation = 'source-over';
        gstars.forEach(s => { const a2 = s.a + cosmosRot * 0.35; const rr = s.r * gR; const x = cx + Math.cos(a2) * rr, y = cy + Math.sin(a2) * rr * 0.42; xctx.globalAlpha = cosmosFade * s.b; xctx.fillStyle = s.c; xctx.beginPath(); xctx.arc(x, y, s.sz, 0, 6.28); xctx.fill(); }); xctx.globalAlpha = cosmosFade;
        xctx.globalCompositeOperation = 'lighter'; for (let band = 0; band < 3; band++) { xctx.beginPath(); const baseY = h * (0.05 + band * 0.045); xctx.moveTo(0, 0); for (let px = 0; px <= w; px += 12) { const xn = px / w; const y = baseY + Math.sin(xn * 9 + auroraPh * 1.4 + band + cosmosRot * 0.6) * 18 + Math.sin(xn * 20 - auroraPh) * 7; xctx.lineTo(px, y); } xctx.lineTo(w, 0); xctx.closePath(); const ag = xctx.createLinearGradient(0, 0, 0, h * 0.34); ag.addColorStop(0, 'rgba(' + ['80,230,160', '120,200,255', '180,120,255'][band] + ',' + (0.16 * cosmosFade) + ')'); ag.addColorStop(1, 'transparent'); xctx.fillStyle = ag; xctx.fill(); } xctx.globalCompositeOperation = 'source-over'; xctx.globalAlpha = cosmosFade;
        planets.forEach(pl => { const a2 = pl.ang + cosmosRot * pl.sp; const x = cx + Math.cos(a2) * pl.dist * gR, y = cy + Math.sin(a2) * pl.dist * gR * 0.5; const pr = pl.r * gR; if (pl.ring) { xctx.save(); xctx.translate(x, y); xctx.rotate(-0.5); xctx.strokeStyle = 'rgba(220,200,150,' + (0.5 * cosmosFade) + ')'; xctx.lineWidth = pr * 0.34; xctx.beginPath(); xctx.ellipse(0, 0, pr * 1.9, pr * 0.7, 0, 0, 6.28); xctx.stroke(); xctx.restore(); } const pg = xctx.createRadialGradient(x - pr * 0.4, y - pr * 0.4, 0, x, y, pr * 1.3); pg.addColorStop(0, pl.col); pg.addColorStop(1, '#0a0a14'); xctx.fillStyle = pg; xctx.beginPath(); xctx.arc(x, y, pr, 0, 6.28); xctx.fill(); });
        const ER = Math.min(w, h) * 0.17; const eg = xctx.createRadialGradient(cx, cy, ER * 0.75, cx, cy, ER * 1.9); eg.addColorStop(0, 'rgba(90,170,255,' + (0.6 * cosmosFade) + ')'); eg.addColorStop(1, 'transparent'); xctx.fillStyle = eg; xctx.fillRect(cx - ER * 2, cy - ER * 2, ER * 4, ER * 4);
        xctx.save(); xctx.beginPath(); xctx.arc(cx, cy, ER, 0, 6.28); xctx.clip();
        const og = xctx.createRadialGradient(cx - ER * 0.35, cy - ER * 0.35, ER * 0.1, cx, cy, ER * 1.3); og.addColorStop(0, '#3a86ee'); og.addColorStop(0.65, '#0d3a72'); og.addColorStop(1, '#04122b'); xctx.fillStyle = og; xctx.fillRect(cx - ER, cy - ER, ER * 2, ER * 2);
        for (let i = 0; i < 40; i++) { const lon = i * 1.13 + cosmosRot * 1.3; const lat = Math.sin(i * 1.7) * 1.25; const depth = Math.cos(lon) * Math.cos(lat); if (depth < 0) continue; const x = cx + Math.sin(lon) * Math.cos(lat) * ER, y = cy + Math.sin(lat) * ER; xctx.globalAlpha = cosmosFade * (0.3 + depth * 0.6); xctx.fillStyle = i % 5 < 3 ? 'rgba(58,150,90,.9)' : 'rgba(150,130,80,.85)'; xctx.beginPath(); xctx.ellipse(x, y, ER * 0.15 * (0.4 + depth), ER * 0.11 * (0.4 + depth), 0, 0, 6.28); xctx.fill(); }
        xctx.globalAlpha = cosmosFade * 0.8; xctx.fillStyle = 'rgba(235,245,255,.85)'; xctx.beginPath(); xctx.ellipse(cx, cy - ER * 0.92, ER * 0.5, ER * 0.18, 0, 0, 6.28); xctx.fill(); xctx.beginPath(); xctx.ellipse(cx, cy + ER * 0.92, ER * 0.5, ER * 0.18, 0, 0, 6.28); xctx.fill();
        for (let i = 0; i < 26; i++) { const lon = i * 1.9 - cosmosRot * 0.8; const lat = Math.sin(i * 2.6) * 1.2; const depth = Math.cos(lon) * Math.cos(lat); if (depth < 0) continue; const x = cx + Math.sin(lon) * Math.cos(lat) * ER, y = cy + Math.sin(lat) * ER; xctx.globalAlpha = cosmosFade * (0.18 + depth * 0.4); xctx.fillStyle = 'rgba(255,255,255,.8)'; xctx.beginPath(); xctx.ellipse(x, y, ER * 0.2 * (0.4 + depth), ER * 0.1 * (0.4 + depth), 0, 0, 6.28); xctx.fill(); }
        const sh = xctx.createLinearGradient(cx - ER, 0, cx + ER, 0); sh.addColorStop(0, 'rgba(0,0,12,.66)'); sh.addColorStop(0.5, 'transparent'); xctx.globalAlpha = cosmosFade; xctx.fillStyle = sh; xctx.fillRect(cx - ER, cy - ER, ER * 2, ER * 2);
        xctx.globalAlpha = cosmosFade * 0.5; const spg = xctx.createRadialGradient(cx - ER * 0.4, cy - ER * 0.4, 0, cx - ER * 0.4, cy - ER * 0.4, ER * 0.7); spg.addColorStop(0, 'rgba(255,255,255,.7)'); spg.addColorStop(1, 'transparent'); xctx.fillStyle = spg; xctx.beginPath(); xctx.arc(cx - ER * 0.4, cy - ER * 0.4, ER * 0.7, 0, 6.28); xctx.fill();
        xctx.restore();
        xctx.globalAlpha = cosmosFade * 0.7; xctx.strokeStyle = 'rgba(140,200,255,.6)'; xctx.lineWidth = 2.5; xctx.beginPath(); xctx.arc(cx, cy, ER + 2, 0, 6.28); xctx.stroke(); xctx.globalAlpha = 1;
        if (dominoing) { if (!dStreaks.length) { for (let k = 0; k < 60; k++) { const ang = Math.random() * 6.28, d = Math.max(w, h) * 0.8; dStreaks.push({ x: cx + Math.cos(ang) * d, y: cy + Math.sin(ang) * d, a: ang, sp: Math.random() * 16 + 14, c: ['#7c6cff', '#5ec8ff', '#e878f5', '#f5b642', '#5ee6c0'][k % 5], life: 1 }); } } xctx.globalCompositeOperation = 'lighter'; dStreaks.forEach(s => { const px = s.x, py = s.y; s.x -= Math.cos(s.a) * s.sp; s.y -= Math.sin(s.a) * s.sp; s.life -= 0.012; if (Math.hypot(s.x - cx, s.y - cy) < 50) s.life = 0; xctx.globalAlpha = Math.max(0, s.life) * 0.75; xctx.strokeStyle = s.c; xctx.lineWidth = 2; xctx.shadowColor = s.c; xctx.shadowBlur = 10; xctx.beginPath(); xctx.moveTo(px, py); xctx.lineTo(s.x, s.y); xctx.stroke(); }); xctx.shadowBlur = 0; xctx.globalCompositeOperation = 'source-over'; xctx.globalAlpha = 1; } });

      if (!mob) {   // no WebGPU raymarch on phones — the static cosmos above replaces it
        const gpuC = this.h('canvas', 'position:absolute;inset:0;width:100%;height:100%;z-index:1;pointer-events:none;'); wrap.appendChild(gpuC);
        const gpuNote = this.h('div', 'position:absolute;top:12px;right:14px;z-index:5;font-family:' + F.mono + ';font-size:9px;letter-spacing:.1em;color:#5a5a68;', { text: '' }); wrap.appendChild(gpuNote);
        this.cosmosWebGPU(gpuC, () => ({ rot: cosmosRot, fade: cosmosFade, px: 0.5, py: 0.5, paused: !inView })).then(ok => { if (ok) { cosmos.style.display = 'none'; gpuNote.textContent = '◈ WebGPU · WGSL raymarch'; gpuNote.style.color = '#5ee6c0'; } else { gpuC.remove(); gpuNote.textContent = '2D fallback'; } }).catch(() => { gpuC.remove(); gpuNote.textContent = '2D fallback'; });
      }
      ...
      const ring = this.h('div', 'position:absolute;inset:0;transform-style:preserve-3d;transition:transform .6s cubic-bezier(.16,1,.3,1);'); tiltBox.appendChild(ring);
      ...
      function domino() {
        if (dominoing || intro) return; dominoing = true; const SLOW = 140;
        cards.forEach((c, k) => { setTimeout(() => { if (!dominoing) return; c.style.transition = 'transform 1.1s cubic-bezier(.5,0,.8,1),opacity 1.1s'; c.style.transform += ' translateY(260px) rotateX(82deg)'; c.style.opacity = '0'; }, k * SLOW); });
      ...
      let sx = 0, down = false; wrap.addEventListener('pointerdown', e => { down = true; sx = e.clientX; }); wrap.addEventListener('pointerup', e => { if (!down) return; down = false; const dx = e.clientX - sx; if (dx > 40) step(-1); else if (dx < -40) step(1); });
      // tilt & pan the whole ring toward the pointer
      wrap.addEventListener('pointermove', e => { if (intro || !useTilt) return; const r = wrap.getBoundingClientRect(); const px = (e.clientX - r.left) / r.width - 0.5, py = (e.clientY - r.top) / r.height - 0.5; tiltBox.style.transform = 'rotateX(' + (-py * 12) + 'deg) rotateY(' + (px * 12) + 'deg)'; }); wrap.addEventListener('pointerleave', () => tiltBox.style.transform = 'rotateX(0deg) rotateY(0deg)');
