...
(function () {
  ...
  var ROLES = [
    { i: 0, num: '01', yr: "'17–'21", org: 'Cal Poly Humboldt', role: 'B.S. Computer Science', loc: 'Arcata, CA', color: '#7c6cff', metric: null, bullets: ['CS degree — the foundation everything compiles from.', 'Bilingual EN / FR (Lycée Français La Pérouse), fluent RU.', 'First taste of systems thinking and shipping working code.'] },
    ...
    { i: 3, num: '04', yr: "'21–'25", org: 'LendingClub', role: 'Release Engineer', loc: 'SF · Hybrid', color: '#7c6cff', metric: ['−30%', 'deploy time'], bullets: ['Led biweekly production releases — dry runs on off-weeks.', 'CI/CD pipelines (GitHub Actions, Jenkins) — cut deploy time 30%.', 'Playwright self-healing test automation as a release gate.', 'AWS EC2 redundancy + failover for zero-downtime deploys.'] },
    { i: 4, num: '05', yr: "'25–", org: 'Independent', role: 'Full-Stack Developer', loc: 'San Francisco', color: '#5ee6c0', metric: ['13', 'shipped solo'], live: true, bullets: ['Designed, built & shipped 13 production apps solo, end to end.', 'Fintech P2P lending + on-chain payments (USDC/USDT/TON/Solana).', 'Algorithmic trading, civic-data AI agents, MV3 Chrome extensions.'] }
  ];
  var BUILDS = [
    ...
    { n: '11', slug: 'civicquery', title: 'CivicQuery', cat: 'Civic AI agent', accent: '#62b6ff', from: [4], isNew: true, lede: 'Ask San Francisco a question in plain English; an LLM agent plans, queries live DataSF, and streams back prose, a chart, and a live map — showing every query it ran. Runs live here.', facts: [['658', 'live datasets'], ['4', 'agent tools'], ['0', 'databases']] },
    { n: '12', slug: 'userscriptpro', title: 'UserScriptPro', cat: 'Dev tooling', accent: '#c792ea', from: [4], isNew: true, lede: 'A next-gen userscript manager on the sanctioned MV3 userScripts API — full GM_* parity, an AI copilot, a visual recorder, per-script trust scores, and a time-travel debugger.', facts: [['MV3', 'userScripts'], ['GM_*', 'full API'], ['7', 'scripts scored']] },
    { n: '13', slug: 'cookielens', title: 'CookieLens', cat: 'Privacy', accent: '#f5b642', from: [4], isNew: true, lede: 'An MV3 extension that x-rays every cookie and hidden tracker on a page — names the company behind each, flags the HttpOnly/third-party trackers the page itself can’t see, and grades the site, all analyzed locally. This demo replays a real run across 79 sites.', facts: [['79', 'sites scanned'], ['1,228', 'trackers found'], ['41', 'graded F']] }
  ];
  // same-origin relative demo paths (match the existing Builds grid embeds; work on Railway + Netlify).
  // civicquery = a same-origin shim that embeds the live Railway deployment.
  var SRC = { sokoloff: 'demos/sokoloff/', freqtrade: 'demos/freqtrade/', stakeodds: 'demos/stakeodds/#demo', loansy: 'demos/loansy/', claudedown: 'demos/claudedown/', 'flex-lane': 'demos/flex-lane/', renovation: 'demos/renovation-watchlist/', urbanforest: 'demos/urbanforest/', 'hidden-sf': 'demos/hidden-sf/', 'sf-on-screen': 'demos/sf-on-screen/', civicquery: 'demos/civicquery/', userscriptpro: 'demos/userscriptpro/', cookielens: 'demos/cookielens/' };
  ...
  function rolePanelHTML(r, withChips) {
    return '<div style="font-family:\'Geist Mono\',monospace;font-size:11px;color:' + r.color + ';letter-spacing:.1em;text-transform:uppercase;">' + r.num + ' · ' + r.yr + (r.live ? ' · <span style="color:#5ee6c0;">● now</span>' : '') + '</div>'
      + '<div style="font-family:\'Space Grotesk\',sans-serif;font-weight:700;font-size:23px;color:#ede8e0;margin:8px 0 3px;letter-spacing:-.02em;">' + esc(r.org) + '</div>'
      + '<div style="font-size:13.5px;color:#a9a39a;margin-bottom:15px;">' + esc(r.role) + ' · ' + esc(r.loc) + '</div>'
      + (r.metric ? '<div style="display:flex;align-items:baseline;gap:9px;margin-bottom:15px;"><span style="font-family:\'Space Grotesk\',sans-serif;font-weight:700;font-size:34px;color:#3fe0d0;letter-spacing:-.03em;">' + r.metric[0] + '</span><span style="font-family:\'Geist Mono\',monospace;font-size:11px;color:#8b857b;">' + r.metric[1] + '</span></div>' : '')
      ...
      + (withChips ? '<div style="margin-top:16px;font-family:\'Geist Mono\',monospace;font-size:11px;color:#8b857b;text-transform:uppercase;letter-spacing:.06em;">stations on this branch</div><div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:9px;">' + builtBy(r.i).map(function (b) { return '<button type="button" data-go="' + b.slug + '" style="cursor:pointer;font-family:\'Geist Mono\',monospace;font-size:11px;color:#cfcfe0;background:#1d1b24;border:1px solid ' + b.accent + '55;border-radius:20px;padding:5px 10px;">' + esc(b.title) + '</button>'; }).join('') + '</div>' : '');
  }
  function buildPanelHTML(b) {
    return '<div style="font-family:\'Geist Mono\',monospace;font-size:11px;color:' + b.accent + ';letter-spacing:.1em;text-transform:uppercase;">' + b.n + ' · ' + esc(b.cat) + (b.isNew ? ' · <span style="color:#5ee6c0;">new</span>' : '') + '</div>'
      + '<div style="font-family:\'Space Grotesk\',sans-serif;font-weight:700;font-size:22px;color:#ede8e0;margin:8px 0 6px;letter-spacing:-.02em;">' + esc(b.title) + '</div>'
      + '<p style="font-size:13px;color:#a9a39a;line-height:1.55;margin:0 0 13px;">' + esc(b.lede) + '</p>'
      + '<button type="button" data-launch="' + b.slug + '" style="cursor:pointer;font-family:\'Geist Mono\',monospace;font-size:12.5px;color:#ede8e0;background:' + b.accent + '1f;border:1px solid ' + b.accent + '66;border-radius:9px;padding:11px 15px;display:inline-flex;align-items:center;gap:8px;outline:2px solid #f5b642;outline-offset:2px;animation:coachBreathe 2.8s ease-in-out infinite;"><span style="display:inline-grid;place-items:center;width:18px;height:18px;border-radius:50%;background:' + b.accent + ';color:#06060a;font-size:8px;">▶</span> launch live app</button>'
      + '<div style="margin-top:15px;font-family:\'Geist Mono\',monospace;font-size:11px;color:#8b857b;text-transform:uppercase;letter-spacing:.06em;">skills feeding it</div>'
      + '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:9px;">' + b.from.map(function (i) { return '<button type="button" data-role2="' + i + '" style="cursor:pointer;font-family:\'Geist Mono\',monospace;font-size:11px;color:#cfcfe0;background:#1d1b24;border:1px solid rgba(237,232,224,.14);border-radius:20px;padding:5px 10px;">' + esc(ROLES[i].org) + '</button>'; }).join('') + '</div>';
  }
  ...
  function setupLiveDock() {
    ...
    var dock = el('div', 'position:fixed;left:0;right:0;top:0;height:0;z-index:9000;overflow:hidden;background:#100e15;border-bottom:1px solid rgba(237,232,224,.1);display:flex;flex-direction:column;');
    ...
    dock.innerHTML = '<div style="height:100%;width:100%;max-width:1340px;margin:0 auto;padding:12px clamp(14px,3vw,40px) 16px;display:flex;flex-direction:column;">'
      ...
      + '<span id="tmLiveAddr" style="font-family:\'Geist Mono\',monospace;font-size:12px;color:#a9a39a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;"></span>'
      + '<a id="tmLiveOpen" target="_blank" rel="noopener" style="font-family:\'Geist Mono\',monospace;font-size:11.5px;color:#cfcfe0;text-decoration:none;border:1px solid rgba(237,232,224,.18);border-radius:7px;padding:6px 11px;flex:none;">open ↗</a>'
      + '<button id="tmLiveClose" type="button" aria-label="Close live app" style="cursor:pointer;font-family:\'Geist Mono\',monospace;font-size:11.5px;color:#cfcfe0;background:transparent;border:1px solid rgba(237,232,224,.18);border-radius:7px;padding:6px 11px;flex:none;">✕ close</button>'
      + '</div>'
      + '<div style="flex:1;min-height:0;border:1px solid rgba(237,232,224,.14);border-radius:12px;overflow:hidden;background:#14131a;position:relative;">'
      + '<iframe id="tmLiveFrame" title="Live app preview" sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-popups-to-escape-sandbox allow-modals" referrerpolicy="no-referrer" style="width:100%;height:100%;border:0;display:block;background:#14131a;"></iframe>'
      + '<div id="tmLiveLoad" style="position:absolute;inset:0;display:none;place-items:center;background:#14131a;font-family:\'Geist Mono\',monospace;font-size:12.5px;color:#8b857b;text-align:center;padding:20px;"><span><span id="tmLoadT">loading …</span><br><span style="font-size:11px;color:#5b5b6e;">if it doesn\'t appear, the app blocks embedding — use <b style="color:#a9a39a;">open ↗</b></span></span></div>'
      + '</div></div>';
  ...
  function makeDesktop(root, live) {
    ...
    BUILDS.forEach(function (b, k) {
      ...
      var st = mk('circle', { cx: bx, cy: by, r: 8, fill: '#14131a', stroke: b.accent, 'stroke-width': 2.6, cursor: 'pointer' }); st.dataset.slug = b.slug; svg.appendChild(st);
    ...
    ROLES.forEach(function (r) {
      var x = rx(r.i);
      svg.appendChild(mk('rect', { x: x - 82, y: ly + 29, width: 164, height: 38, rx: 8, fill: '#100e15', opacity: .82, 'pointer-events': 'none' }));
      var ring = mk('circle', { cx: x, cy: ly, r: 13, fill: '#14131a', stroke: r.color, 'stroke-width': 3, cursor: 'pointer' }); ring.dataset.role = r.i; svg.appendChild(ring);
      svg.appendChild(mk('circle', { cx: x, cy: ly, r: 5, fill: r.color, 'pointer-events': 'none' }));
      var t1 = mk('text', { x: x, y: ly + 40, fill: '#ede8e0', 'font-size': 14, 'font-family': 'Space Grotesk, sans-serif', 'font-weight': 600, 'text-anchor': 'middle', cursor: 'pointer' }); t1.dataset.role = r.i; t1.textContent = r.org; svg.appendChild(t1);
  ...
  function makeMobile(root, live) {
    ...
    BUILDS.forEach(function (b, k) {
      ...
      branches.push({ el: bp, color: b.accent, fireColor: FIRE[k % FIRE.length] });
      var st = mk('circle', { cx: bx, cy: by, r: 8, fill: '#14131a', stroke: b.accent, 'stroke-width': 2.6, cursor: 'pointer' }); st.dataset.slug = b.slug; svg.appendChild(st);
      var lab = mk('text', { x: right ? bx + 14 : bx - 14, y: by + 4, fill: '#cfcfe0', 'font-size': 11, 'font-family': 'Geist Mono, monospace', 'text-anchor': right ? 'start' : 'end', cursor: 'pointer' }); lab.dataset.slug = b.slug; lab.textContent = shortM(b.title); svg.appendChild(lab);
    ...
    ROLES.forEach(function (r) {
      var y = ry(r.i);
      svg.appendChild(mk('rect', { x: lx - 76, y: y - 43, width: 152, height: 23, rx: 7, fill: '#100e15', opacity: .86, 'pointer-events': 'none' }));
      svg.appendChild(mk('rect', { x: lx - 58, y: y + 22, width: 116, height: 23, rx: 7, fill: '#100e15', opacity: .86, 'pointer-events': 'none' }));
      svg.appendChild(mk('rect', { x: lx - 24, y: y - 18, width: 48, height: 36, rx: 9, fill: '#100e15', 'pointer-events': 'none' }));
      var ring = mk('circle', { cx: lx, cy: y, r: 14, fill: '#14131a', stroke: r.color, 'stroke-width': 3, cursor: 'pointer' }); ring.dataset.role = r.i; svg.appendChild(ring);
      svg.appendChild(mk('circle', { cx: lx, cy: y, r: 5.5, fill: r.color, 'pointer-events': 'none' }));
      var t1 = mk('text', { x: lx, y: y - 26, fill: '#ede8e0', 'font-size': 15, 'font-family': 'Space Grotesk, sans-serif', 'font-weight': 700, 'text-anchor': 'middle', cursor: 'pointer' }); t1.dataset.role = r.i; t1.textContent = r.org; svg.appendChild(t1);
      var t2 = mk('text', { x: lx, y: y + 34, fill: '#8b857b', 'font-size': 10.5, 'font-family': 'Geist Mono, monospace', 'text-anchor': 'middle', 'pointer-events': 'none' }); t2.textContent = r.num + ' · ' + r.yr; svg.appendChild(t2);
    });
    var stop = trainAnim(svg, mainLine, branches);
